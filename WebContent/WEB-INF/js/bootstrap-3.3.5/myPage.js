(function($){
	$.fn.extend({
		"my_page":function(form){
		    ...
		    //定义分页样式结构体
		    var $pageStyle={
		    		styleLaquo_ : 'laquo',//上一页样式
					styleRaquo_ : 'raquo',//下一页样式
		    }
		    ...
		    if($pageInfo.currentPage == 0 || $pageInfo.currentPage == 1){
		    	$pageStyle.styleLaquo_='disabled';
		    }else if($pageInfo.currentPage == $pageInfo.pageCount){
		    	$pageStyle.styleRaquo_='disabled';
		    }
		    ...
		    //绑定数据处理函数
		    $this.find("ul li[class=\'laquo\'] a").bind("click",function(){
		    	alert('up');
...
//		                  redirectTo(pageinfo.currentPage-1);
		    ...
		    $this.find("ul li[class=\'raquo\'] a").bind("click",function(){
		    	alert('xia');
